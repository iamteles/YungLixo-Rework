...
class OriginalChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var reloadSong:FlxButton = new FlxButton(saveButton.x + saveButton.width + 10, saveButton.y, "Load Normal", function()
		{
			loadSong(_song.song, false);
		});

		var reloadSongReshaped:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 30, "Load Reshaped", function()
		{
			loadSong(_song.song, true);
		});

		var reloadSongJson:FlxButton = new FlxButton(reloadSongReshaped.x, reloadSongReshaped.y + 30, "Reload JSON", function()
		{
			loadJson(_song.song.toLowerCase());
		});

		var loadAutosaveBtn:FlxButton = new FlxButton(saveButton.x, saveButton.y + 30, 'Load Autosave', loadAutosave);
		...
		tab_group_song.add(reloadSongReshaped);
	...
	function addNoteUI():Void
	{
		...
		// note types
		stepperType = new FlxUINumericStepper(10, 30, 1, 0, 0, 3); // 10 is placeholder
	...
	function loadSong(daSong:String, ?isReshaped:Bool):Void
	{
		if (songMusic != null)
			songMusic.stop();
		...
		if(isReshaped)
		{
			songMusic = new FlxSound().loadEmbedded(Paths.instReshaped(daSong), false, true);
			if (_song.needsVoices)
				vocals = new FlxSound().loadEmbedded(Paths.voicesReshaped(daSong), false, true);
			else
				vocals = new FlxSound();
		}
		else
